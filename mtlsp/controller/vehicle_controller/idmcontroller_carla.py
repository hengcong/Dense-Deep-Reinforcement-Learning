...
class IDMController(DiscreetController):
    def __init__(self, observation_method=ObservationCarla, env=None, controllertype="IDMController"):
    ...
    #@staticmethod
    def decision_pdf(self,observation):
        ...
        """
        action, mode = self.decision(observation)
        pdf = [0, 0, 0]
    ...
    def decision(self, observation):
        ...
        # Longitudinal: IDM
            ...
            mode = "IDM"
            if ego_vehicle["veh_id"]==self.env.ego_vehicle_wrapper.id:
                veh_type = "CAV"
            else:
                veh_type = "BV"
    ...
    def mobil_gain(self, lane_index, observation):
        ...
        if ego["veh_id"] ==self.env.ego_vehicle_wrapper.id:
            mode = "CAV"
        else:
            mode = "BV"
        ...
        gain = self_pred_a - self_a + CAV_POLITENESS * (
    ...
    def step(self):
        ...
        """
        super().step()
        self.action, self.mode = self.decision(self.vehicle_wrapper.observation.information)
