...
from .nddglobalcontrollercarla import NDDBVGlobalController
...
class NADEBVGlobalController(NDDBVGlobalController):
    ...
    def step(self):
        ...
        self.update_controlled_vehicles(controller=TreeSearchNADEBackgroundController)
        ...
        if conf.experiment_config["mode"] == "NDE":
            ...
            for bv_id in self.controllable_veh_id_list:
                vw = self.env.vehicle_wrapper_list[bv_id]
                vw.update_observation(self.env)
                vw.controller.step()
                vw.update(self.env)
            ...
            # ego_obs = vw.observation.information.get("Ego", None)
                # if ego_obs is None:
                #     print(f"[❌ MISSING OBS] Vehicle {bv_id} has no Ego in observation!")
                # else:
                #     print(f"[✓ OBS] Vehicle {bv_id} Ego:", ego_obs)
    ...
    def update_controlled_vehicles(self, controller=TreeSearchNADEBackgroundController):
        env = self.env
    ...
    def calculate_criticality_list(self, controlled_bvs_list, CAV_obs, full_obs):
        ...
        predicted_full_obs, predicted_traj_obs = NADEBVGlobalController.pre_load_predicted_obs_and_traj(full_obs) # NOTE: @staticmthod, defined but not tested yet
    ...
    def get_Surrigate_CAV_action_probability(cav_obs):
        ...
        for lane_index in lane_index_list:
            LC_safety_flag, gain = NADEBVGlobalController._Mobil_surraget_model(cav_obs, lane_index)
            if gain is not None:
                if lane_index == -1:
                    right_gain = np.clip(gain, 0., None)
                    right_LC_safety_flag = LC_safety_flag
